...
def parseFinam(ticker, period, start="01.05.2020"):
	#пользовательские переменные
	# ticker="SBER" #задаём тикер
	# задаём период. Выбор из: 'tick': 1, 'min': 2, '5min': 3, '10min': 4, '15min': 5, '30min': 6, 'hour': 7, 'daily': 8, 'week': 9, 'month': 10
	# period = 7
	...
	end_month = end.month
	end_day = end.day
	end = '.'.join([str(end_day), str(end_month), str(end_year)])
	...
	# TODO: исправить на полный набор эмитентов с биржи
	tickers={'ABRD':82460,
			 'AESL':181867,
			 'AFKS':19715,
			 'AFLT':29,
	...
	local_file.close()
	print("Готово. Проверьте файл *.txt в папке где лежит скрипт")
